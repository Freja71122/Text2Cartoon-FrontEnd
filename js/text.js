...
function returnResponse() {
  ...
  var answer = (getInput().indexOf('sudo') === 0) ? 'Nice try. No.' : 'Rendering ...';
  if (getInput().indexOf('$clear$') === 0){
    var lis = result.getElementsByClassName('add_info')
    for(var i=lis.length;i>0;i--){
      result.removeChild(lis[i-1]);
    }
  }
  else{
